...
taskQueries.createTask = async (req, res) => {
  try {
    const { title, endDate, startDate, allDay } = req.body
    const task = await Task.create({ title, endDate, startDate, allDay })
    res.status(201).json({ id: task._id, title, endDate, startDate, allDay })
  } catch (err) {
...
taskQueries.updateTask = async (req, res) => {
  try {
    ...
    const task = await Task.findOneAndUpdate({ _id: id }, { $set: updatedTask }, { new: true })
    if (!task) {
